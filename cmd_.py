...
def save_backup():
    ...
    print("backup")
    print(prop.backup_root)
    os.makedirs( prop.backup_dir , exist_ok=True )#バックアップフォルダが無ければ作成
    print(newpath)
...
def setproject(mode):
    ...
    #現在のシーンがどのプロジェクトに属しているか判別
    size = len(bpy.context.scene.cyascenemanager_proj)
    index = -1
    for i in range(size):
        work_root= bpy.context.scene.cyascenemanager_proj[i].work_root

        work_root= os.path.normpath(work_root)
        exists= work_root in filepath
        print(exists,work_root,filepath)
        if(exists):
            prop.work_root=bpy.context.scene.cyascenemanager_proj[i].work_root
            prop.backup_root=bpy.context.scene.cyascenemanager_proj[i].backup_root
            prop.onedrive_root=bpy.context.scene.cyascenemanager_proj[i].onedrive_root

            index = i

            rel_dir = filepath.replace(work_root,'')
